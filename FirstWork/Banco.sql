create database if not exists first_work;

use first_work;

create table if not exists usuario(
id int not null auto_increment,
nome varchar(200) not null,
CPF 
telefone varchar(100) not null,
Data_nascimento date not null,
Endereco varchar(200) not null,
Bairro varchar(200) not null,
Estado varchar(100) not null,
email varchar(100) not null,
descr varchar(200) not null,
img varchar(100) not null,
rede_social varchar(100) not null,
primary key (id));

create table if not exists empresa(
id int not null auto_increment,
CNPJ 
nome varchar(200) not null,
Local varchar(100) not null,
Hora_Funcionamento Int not null,
Logo varchar(100) not null,
Primary key(CNPJ));

create table if not exists vaga(
id_vaga int not null auto_increment,
Cargo varchar(100) not null,
Salario Float not null,
carga_horaria Int not null,
Requisitos varchar(250) not null,
id_usuario int not null,
Cnpj_emp int not null,
primary key(id_vaga),
foreign key(id_usuario) references first_work.usuario(id),
foreign key(Cnpj_emp) references first_work.empresa(CNPJ));

create table if not exists log_vaga(
id_log Int not null auto_increment,
data_cadastro datetime,
reg_cnpj int not null,
reg_vaga int not null,
primary key (id_log),
foreign key (reg_cnpj) references first_work.empresa(CNPJ),
foreign key (reg_vaga) references first_work.vaga(id_vaga));